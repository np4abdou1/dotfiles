...
static void add_info(const char *icon, const char *label, const char *fmt, ...) {
  ...
  if (icon && icon[0])
    snprintf(line, sizeof(line), "\033[38;2;137;192;255m%s\033[0m \033[1;%sm%-10s\033[0m\033[38;2;93;162;232m│\033[0m %s", icon, label_color, label, val);
  else
    snprintf(line, sizeof(line), "\033[1;%sm%s\033[0m: %s", label_color, label, val);
...
static void gather_cpu(void) {
  ...
  if (name[0]) {
    if (cores > 0 && max_ghz > 0)
      add_info("󰻠", "CPU", "%s (%d) @ %.2f GHz", name, cores, max_ghz);
    else if (cores > 0)
      add_info("󰻠", "CPU", "%s (%d)", name, cores);
    else
      add_info("󰻠", "CPU", "%s", name);
  }
}
...
static void gather_memory(void) {
  ...
  add_info("󰾆", "Memory", "%.2f GiB / %.2f GiB (\033[%sm%d%%\033[0m)", used_gib,
           total_gib, color, pct);
}
...
static void gather_disk(void) {
  ...
  if (fstype[0])
    add_info("󰋊", "Disk (/)", "%.2f GiB / %.2f GiB (\033[%sm%d%%\033[0m) - %s",
             used_gib, total_gib, color, pct, fstype);
  else
    add_info("󰋊", "Disk (/)", "%.2f GiB / %.2f GiB (\033[%sm%d%%\033[0m)", used_gib,
             total_gib, color, pct);
}
...
static void gather_battery(void) {
  ...
  if (capacity >= 0) {
    ...
    if (time_str[0] && status[0])
      add_info("󱈑", "Battery", "\033[%sm%d%%\033[0m (%s) [%s]", color, capacity,
               time_str, status);
    else if (status[0])
      add_info("󱈑", "Battery", "\033[%sm%d%%\033[0m [%s]", color, capacity, status);
    else
      add_info("󱈑", "Battery", "\033[%sm%d%%\033[0m", color, capacity);
  }
}
...
static void gather_theme(void) {
  ...
  if (theme[0]) {
    ...
    if (strlen(theme) > 20) theme[20] = '\0';
    add_info("", "Theme", "%s", theme);
  }
}
...
// Default colors: dark blue (outer) + light blue (inner)
static const char *color_inner = "\033[38;2;137;192;255m";
static const char *color_outer = "\033[38;2;93;162;232m";
...
static void set_distro_colors(const char *distro) {
  if (strcasecmp(distro, "gentoo") == 0) {
    ...
    color_outer = "\033[38;2;93;162;232m";   // dark blue
    color_inner = "\033[38;2;137;192;255m";  // light blue
  ...
  for (int frame = 0; max_frames == 0 || frame < max_frames; frame++) {
    ...
    for (int i = 0; i < POINT_COUNT; i++) {
      ...
      if (ooz > zbuf[ys][xs]) {
        ...
        if (ci > smax) ci = smax;
        shade_idx[ys][xs] = ci;
        colorbuf[ys][xs] = (PWEIGHT[i] >= color_threshold) ? 1 : 0;
      }
    }
